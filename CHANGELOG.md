...
- **Learned rule text quality.** Human-pattern synth is **extract → cluster**. Learned rules are **titled** (`title` + `body` packed in `rule_text`). Concrete cues live in the body when useful — no separate smell field / `Look for:` / inject `Flag:` ceremony. Soft/org conventions stay Pending for admins. Gate: title + body (size bounds). Path scope LLM-chosen, shallow-capped (≤2 segments). Admin **Rebuild** clears auto-synth Pending first (keeps `@remember`). Rejected near-dupes are not recreated. Deterministic **reject_pattern** rows are also titled packs so they inject. Env: `MIRA_HUMAN_SYNTH_MAX_RULES` (40), `MIRA_HUMAN_SYNTH_EXTRACT_CAP` (80), `MIRA_HUMAN_SYNTH_EXTRACT_BATCH` (10).
